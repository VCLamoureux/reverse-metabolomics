...
setwd("OneDrive-UniversityofCalifornia,SanDiegoHealth/")
...
# import csv file containing USI and compound name in a two column tables
## it can also be use as the input on the FASST batch workflow on GNPS2
input_FASST_red <- fread("Postdoc_UCSD/Postdoc_projects/Main_project/Drug_screening/Microbial_drug_metabolism_FASST_output_18janv2025/Batch_upload_Microbial_metabolism_of_drugs.csv")

# import the FASST output file
fasst_batch_output <- fread("Postdoc_UCSD/Postdoc_projects/Main_project/Drug_screening/Microbial_drug_metabolism_FASST_output_18janv2025/e68aaef66b2a4bc8889daca90825c7ce-output.tsv")

# add a filtering step - define my users
fasst_batch_output_filtered <- fasst_batch_output %>%
  dplyr::filter(Cosine > 0.8) |> 
  dplyr::filter(`Matching Peaks` >= 2)

fasst_batch <- left_join(fasst_batch_output_filtered, input_FASST_red, by = c("query_usi" = "input_usi"))
#fasst_batch <- left_join(fasst_batch_BA_unique_delta_mass_output, Table_1_input_FASST_BA_unique_delta_mass_red, by = c("query_usi" = "SpectrumID"))

# Add Compound column based on query_usi values
molecules_interest_batch_mode <- fasst_batch |> 
  dplyr::rename(Compound = input_name) |>
